...
var srcCss = ["lib/css/sequenceViewer.css"]
...
gulp.task("scriptsComplete", function () {
    return gulp.src(srcBundle)
        .pipe(concat("pdb.sequence.viewer.bundle.min.js"))
        .pipe(uglify())
		.pipe(header(license, {} ))
		.pipe(header(banner, {} ))
...
gulp.task("scriptsCore", function () {
    return gulp.src(srcCore)
        .pipe(concat("pdb.sequence.viewer.min.js"))
        .pipe(uglify())
		.pipe(header(license, {} ))
		.pipe(header(banner, {} ))
        .pipe(gulp.dest("./build"));
...
gulp.task('stylesCore', function () {
    gulp.src(srcCss)
        ...
        .pipe(concat('pdb.sequence.viewer.min.css'))
        .pipe(cleanCSS())
		.pipe(header(license, {} ))
		.pipe(header(banner, {} ))
        .pipe(gulp.dest("./build"));
...
gulp.task("default", ["clean", "scriptsCore", "scriptsComplete", "stylesCore"]);
